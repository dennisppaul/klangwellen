...
#define KLANG_SAMPLING_RATE           44100
...
static const int mNumSamples = klangwellen::KlangWellen::DEFAULT_SAMPLING_RATE * 4;  // 4 seconds
...
int main() {
    ...
    for (size_t i = 0; i < mNumSamples; i++) {
        if (i % (klangwellen::KlangWellen::DEFAULT_SAMPLING_RATE / 4) == 0) {
    ...
    write_WAV_file("klangwellen-to-wav.wav",
                   klangwellen::KlangWellen::DEFAULT_SAMPLING_RATE,
                   1,
                   mAudioBuffer,
                   mNumSamples);
